...
**/
    ...
    struct tBTNode *docasny_uzol = *RootPtr;
    struct tBTNode *pred_uzol = NULL;

    //cyklus pre najdutie pripadne existujuceho prvku
    while (docasny_uzol != NULL) {
        pred_uzol = docasny_uzol;

        if (Content > docasny_uzol->Cont) {
            docasny_uzol = docasny_uzol->RPtr;
        } else if (Content < docasny_uzol->Cont) {
            docasny_uzol = docasny_uzol->LPtr;
        } else if (Content == docasny_uzol->Cont) {
            return;
        }
    }

    // nenasli sme uz vytvoreny prvok tak si ho vytvorime
    struct tBTNode *novy_uzol = malloc(sizeof(struct tBTNode));

    if (novy_uzol == NULL) {
        return;
    }

    novy_uzol->Cont = Content;
    novy_uzol->RPtr = NULL;
    novy_uzol->LPtr = NULL;

    // nastavenie noveho uzla na koren ak predchadazuci koren je null
    if (pred_uzol == NULL) {
        *RootPtr = novy_uzol;
    } else if (Content > pred_uzol->Cont) {
        pred_uzol->RPtr = novy_uzol;
    } else if (Content < pred_uzol->Cont) {
        pred_uzol->LPtr = novy_uzol;
    }
}
...
**/
    ...
    do {
        if ((*RootPtr) == NULL) {
            ...
            struct tBTNode *docastny_koren = (*RootPtr);
